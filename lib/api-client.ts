...
class ApiClient {
  ...
  async createSession(workspaceId: string, data: CreateSessionRequest): Promise<Session> {
    return this.fetch<Session>(`/workspaces/${workspaceId}/sessions`, {
      method: "POST",
      body: JSON.stringify(data),
    })
  ...
  // Terminal
  async executeCommand(command: string, sessionId?: string): Promise<TerminalExecuteResponse> {
  ...
  async getTerminalHistory(): Promise<{ type: "input" | "output"; content: string }[]> {
    return this.fetch("/terminal/history")
  }

  // Suggestions
  async getSuggestions(query: string, type?: "path" | "repo"): Promise<Suggestion[]> {
