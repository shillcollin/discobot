...
function parseWorkspacePath(path: string, sourceType: "local" | "git") {
  ...
  const githubSshMatch = path.match(/git@github\.com:([^/]+\/[^/]+?)(?:\.git)?$/)
  ...
  const stripped = path.replace(/^(https?:\/\/|git@|ssh:\/\/)/, "").replace(/\.git$/, "")
...
export function SidebarTree({
  ...
  const [expandedIds, setExpandedIds] = React.useState<Set<string>>(new Set(["ws-1"]))
  ...
  return (
    <div className={cn("flex flex-col overflow-hidden", className)}>
      <div className="px-3 py-2 border-b border-sidebar-border flex items-center justify-between">
        <span className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Workspaces</span>
...
function WorkspaceNode({
  ...
  const { displayPath, isGitHub } = parseWorkspacePath(workspace.path, workspace.sourceType)

  const visibleSessions = showClosed ? workspace.sessions : workspace.sessions.filter((s) => s.status !== "closed")
...
function SessionNode({
  ...
  return (
    ...
    >
      <button onClick={() => onSessionSelect(session)} className="flex items-center gap-1.5 min-w-0 flex-1">
        <span className="shrink-0 flex items-center justify-center w-4 h-4">{getStatusIndicator(session.status)}</span>
