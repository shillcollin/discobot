    ...
    <div className={cn("flex flex-col h-full bg-background", className)}>
      ...
      <div className="flex items-center border-b border-border bg-muted/30 overflow-x-auto shrink-0">
        {openFiles.map((file) => (
          ...
          >
            <FileCode className={cn("h-4 w-4", file.changed ? "text-green-500" : "text-sky-500")} />
  ...
  const diffLines = React.useMemo(() => {
    ...
    for (let i = 0; i < maxLines; i++) {
      ...
      if (original === undefined && current !== undefined) {
        ...
        result.push({ type: "unchanged", lineNumber: i + 1, content: current || "" })
      }
    }
        ...
        <table className="w-full border-collapse">
          <tbody>
            {diffLines.map((line, idx) => (
              <tr
                key={idx}
                className={cn(line.type === "added" && "bg-green-500/10", line.type === "removed" && "bg-red-500/10")}
              >
