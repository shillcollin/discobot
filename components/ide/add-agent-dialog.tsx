...
import type { Agent, CreateAgentRequest, MCPServer, MCPServerConfig, SupportedAgentType } from "@/lib/api-types"
...
function MCPServerEditor({
  ...
  const parsed = React.useMemo(() => parseCommandLine(commandLine), [commandLine])
    ...
    <div className="border rounded-lg p-3 space-y-3 bg-muted/30">
      ...
      {server.config.type === "stdio" ? (
          ...
          <div>
            ...
            <Textarea
              ...
              onChange={(e) => {
                ...
                updateConfig({ env: Object.keys(env).length > 0 ? env : undefined })
              }}
        ...
        <div className="space-y-2">
          ...
          <div>
            <Label className="text-xs text-muted-foreground">Headers (KEY: value, one per line)</Label>
            <Textarea
              ...
              onChange={(e) => {
                ...
                updateConfig({ headers: Object.keys(headers).length > 0 ? headers : undefined })
              }}
...
export function AddAgentDialog({ open, onOpenChange, onAdd, editingAgent }: AddAgentDialogProps) {
  const { agentTypes, isLoading } = useAgentTypes()
  const [selectedType, setSelectedType] = React.useState<SupportedAgentType | null>(null)
  ...
  React.useEffect(() => {
    if (editingAgent && agentTypes.length > 0) {
      ...
      if (editingAgent.systemPrompt || (editingAgent.mcpServers && editingAgent.mcpServers.length > 0)) {
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-[550px] max-h-[85vh] overflow-hidden flex flex-col">
          ...
          <div className="space-y-2">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" className="w-full justify-between bg-transparent" disabled={isLoading}>
                  {selectedType ? (
                    <div className="flex items-center gap-2">
                      <IconRenderer icons={selectedType.icons} className="h-4 w-4" />
                      <span>{selectedType.name}</span>
                    </div>
                  ) : (
                    <span className="text-muted-foreground">{isLoading ? "Loading..." : "Select agent type"}</span>
                  )}
              ...
              <DropdownMenuContent className="w-[500px]" align="start">
                {agentTypes.map((type) => (
                  ...
                  >
                    ...
                    {selectedType?.id === type.id && <Check className="h-4 w-4 text-primary shrink-0" />}
                  </DropdownMenuItem>
                ))}
              </DropdownMenuContent>
            </DropdownMenu>
            {selectedType && selectedType.defaultTools && selectedType.defaultTools.length > 0 && (
              <p className="text-xs text-muted-foreground">Includes: {selectedType.defaultTools.join(", ")}</p>
            )}
          </div>
            ...
            <CollapsibleContent className="space-y-6 pt-4">
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  This will be appended to the agent&apos;s default system prompt.
                </p>
              </div>
        ...
        <DialogFooter className="border-t pt-4">
          ...
          <Button onClick={handleSubmit} disabled={!selectedType || !name.trim() || isSubmitting}>
            {submitButtonText}
          </Button>
        </DialogFooter>
      </DialogContent>
